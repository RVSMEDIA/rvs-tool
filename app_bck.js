...
app.post('/xls-to-csv', upload, async(req, res) => {
  ...
  const file = req.file;
  console.log('results dsfsdfsdfsdf ', file);
  ...
  if (!file) {
   ...
   // Load the XLS file using xlsx
   try {
    const workbook = xlsx.readFile(file.path); // Reads the XLS file
    ...
    // Save the CSV file
    const csvFilePath = path.join(__dirname, 'uploads', `${file.filename}.csv`);
    fs.writeFileSync(csvFilePath, csvData);

    // Send success response with download link (optional)
    res.status(200).render('xls-to-csv', { success: 'File uploaded and converted successfully!', csvPath: csvFilePath });
  } catch (error) {
    console.error('Error processing the file:', error);
    res.status(500).render('xls-to-csv', { error: 'An error occurred during file processing!' });
  } finally {
    // Clean up the uploaded XLS file if desired
    fs.unlinkSync(file.path);
  }

  // Send a success response
  res.status(200).render('xls-to-csv', { success: 'File uploaded successfully!' });
});
